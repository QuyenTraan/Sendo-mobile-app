...
export default class MonAnController implements IBaseController {

    getOne = (req: express.Request, res: express.Response): void => {
        let id = req.query.id_mon_an; ///body dung cho post
        console.log(id)
        if (!id) {
            res.status(400).json({
                error: 'bị lỗi'
    ...
    seek = (req: express.Request, res: express.Response): void => {
        let id = req.query.id; ///body dung cho post
        console.log(id)
        if (!id) {
            res.status(400).json({
                error: 'bị lỗi'
    ...
    search = (req: express.Request, res: express.Response): void => {
        ...
        let promiseResult = this._monanRepo.search(req.body.ten_mon_an_khong_dau)
            .then(result => {
                if (result.rowCount > 0) {
                    let noidung = result.rows;
                    return { result: noidung };
    ...
    getTop = (req: express.Request, res: express.Response): void => {
        ...
        let promiseResult = this._monanRepo.getTop(id)
            .then(result => {
    ...
    save = (req: express.Request, res: express.Response): void => {
        ...
        let promiseResult = this._monanRepo.save(req.body.ids)
            .then(result => {
                if (result.rowCount > 0) {
                    let noidung = result.rows;
                    return { result: noidung };
